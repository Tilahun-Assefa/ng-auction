...
export class HttpProductService implements ProductService {
  ...
  search(searchparams: ProductSearchParams): Observable<Product[]> {
    ...
    let keys = Object.keys(searchparams.params);
    keys.forEach((key) => {
      queryParams = queryParams.set(key, searchparams.params[key].toString());
    });
    return this.http.get<Product[]>(`${this.baseUrl}/api/products`, { params:queryParams });
  }
}
...
export class _StaticProductService implements ProductService {
  ...
  search(params: ProductSearchParams): Observable<Product[]> {   
    ...
    return this.http.get<Product[]>('data/products.json', { params });
  }
